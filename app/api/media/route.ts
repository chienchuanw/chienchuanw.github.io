...
export async function GET(request: NextRequest) {
  try {
    ...
    if (unassociated) {
      ...
      mediaItems = await mediaService.getUnassociatedByAuthorId(user.id);
    } else if (postId) {
      // Get media for a specific post
      mediaItems = await mediaService.getByPostId(postId);
    } else {
      // Get all media for the user
      mediaItems = await mediaService.getByAuthorId(user.id);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Save file
    const savedMedia = await mediaService.saveFile(file, user.id, postId);
